"""
Fixed-area Gaussian grid map for drone local obstacle mapping.
...
class FixedGaussianGridMap:
    ...
    def draw_heatmap(self, ax, cmap: str = 'hot', alpha: float = 0.7):
        """
        Render the occupancy map as a pcolormesh on an existing matplotlib Axes.
        ...
        Returns
        -------
        pcolormesh artist, or None if the map has not been initialised yet.
        """
        if self.gmap is None:
            return None
        ...
        x_edges = np.linspace(
            self.minx - self.reso / 2.0,
            self.minx + self.cells * self.reso - self.reso / 2.0,
            self.cells + 1,
        )
        y_edges = np.linspace(
            self.miny - self.reso / 2.0,
            self.miny + self.cells * self.reso - self.reso / 2.0,
            self.cells + 1,
        )
        pcm = ax.pcolormesh(
            x_edges, y_edges, self.gmap.T,
            vmin=0.0, vmax=1.0,
